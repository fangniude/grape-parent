...
public class ResourceQueryAdapter implements BeanQueryAdapter {
    ...
    public boolean isRegisterFor(Class<?> cls) {
        return GrapeResource.class.isAssignableFrom(cls);
    }
    ...
    public void preQuery(BeanQueryRequest<?> request) {
        ...
        jun.in("id", AccountResourceRelation.findResourceIdsByAccountQuery(aClass.getName(), Account.current()));

        // 3. add role resource relation
        jun.in("id", RoleResourceRelation.findResourceIdsByAccount(aClass.getName(), Account.current()));
